        ...
        let ret = match self {
            ...
            Tree(ref mut node) => match node.key.cmp(&key) {
                ...
                Less => {
                  ...
                  //inserted 表示是否插入，deepened 表示是否加深
                  if deepened {
...
fn postorder<T: Clone + Ord + Debug>(avl: &AvlTree<T>) {
    match avl {
        ...
        Tree(node) => {
            ...
            println!("Postorder key: {:?}", node.key);
        },
    }
}
fn levelorder<T: Clone + Ord + Debug>(avl: &AvlTree<T>) {
    ...
    while !q.is_empty() {
        ...
        match front {
            Null => (),
            Tree(node) => {
                println!("Levelorder key: {:?}", node.key);
                let _r = q.enqueue(&node.left);
                let _r = q.enqueue(&node.right);
            },
        }
