...
impl<T> Graph<T> 
    where T: Hash + Eq + PartialEq + Clone {
    fn new() -> Self {
        Self {
            vertnums: 0,
            edgenums: 0,
...
fn word_ladder (
    ...
    while vertex_queue.len() > 0 {
        ...
        for nbr in curr.get_neighbors() {
            //克隆，避免和图中数据起冲突
            //Graph的vertices用RefCell包裹就不需要克隆
...
fn main() {
    ...
    let words = vec![
        "FOOL", "COOL", "POOL", "FOUL", "FOLL",
        "FALL", "FALL", "POLL", "PALL", "POLE",
        "PALE", "SALE", "PAGE", "SAGE",
    ];
    let len = words.len();
    ...
    println!("Time cost: {:?}ms", time.elapsed().as_millis());
}
